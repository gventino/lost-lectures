...
1. `a` e `b` são eventos no **mesmo processo** e `a` ocorre antes de `b`, **OU**
2. `a` é o **envio** de uma mensagem e `b` é o **recebimento** dessa mensagem, **OU**
3. Existe um evento `c` tal que `a → c` e `c → b` (**transitividade**)
...
> Kleppmann (2017) discute extensivamente o problema de "The Trouble with Distributed
> Systems" no Cap. 8 de *Designing Data-Intensive Applications*, pp. 287–296.
