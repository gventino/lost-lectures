...
| Propriedade              | Lamport Clock           | Vector Clock            |
|--------------------------|-------------------------|-------------------------|
| Estrutura                | 1 contador (inteiro)    | N contadores (um/proc)  |
| Se `a → b`               | `C(a) < C(b)`        | `V(a) < V(b)`        |
| Se `C(a) < C(b)`         | **Não** implica `a → b` | `V(a) < V(b)` ⟺ `a → b`|
| Detecta concorrência     | Não                   | Sim                   |
| Espaço por mensagem      | O(1)                    | O(N) - N = nº processos |

> **Strong Clock Condition:** `V(a) < V(b)` se e somente se `a → b`
>
> Fidge (1988) e Mattern (1989) propuseram esta extensão independentemente.
> Coulouris et al. (2012) apresentam ambas as formulações na Seção 14.4.
...
> Regras definidas em Mattern (1989), Seção 3.1, e equivalentes às
> formuladas por Fidge (1988). Apresentadas em forma unificada em
> Coulouris et al. (2012), Figura 14.7.
...
> Em sistemas grandes, esta propriedade é essencial. Tanenbaum & Van Steen (2017)
> discutem as implicações na Seção 6.2.2.
...
|-----------------------------------|------------------------------------|
| Detecta concorrência              | Tamanho O(N) por mensagem          |
| Relação causal completa           | N = número de processos no sistema |
| Strong Clock Condition            | Não escala para milhares de nós    |
| Sem falsos positivos/negativos    | Mais complexo que Lamport Clock    |
...
> Kleppmann (2017) discute alternativas como *dotted version vectors* e
...
| Conceito            | Descrição                                           |
|---------------------|-----------------------------------------------------|
| Vector Clock        | Vetor de contadores, um por processo                |
| Strong Clock Cond.  | `V(a) < V(b)` ⟺ `a → b`                            |
| TICK                | Incrementa só o local                               |
| ENVIAR              | TICK + snapshot completo do vetor                   |
| RECEBER(ts)         | Max element-wise + incrementa local                 |
| COMPARAR            | Compara dois vetores → relação de causalidade       |
| SNAPSHOT            | Cópia imutável sem alterar o clock                  |
| MERGE               | Combina vetores sem incrementar (para agregadores)  |
| Trade-off           | Detecta tudo, mas é O(N) por mensagem               |
