...
#block(inset: (left: 1em), fill: luma(245), radius: 4pt, width: 100%)[
  A relação *happens-before* é formalmente definida em Lamport (1978), Definição na p. 559. A detecção de concorrência via vector clocks foi introduzida por Fidge (1988) e Mattern (1989) independentemente.
]
...
#table(
  ...
  [*Notação*],
  [Irreflexividade],
  [Um evento não happens-before si mesmo],
  [¬(a → a)],
  [Antissimetria],
  [Se `a → b`, então ¬(`b → a`)],
  [Direção única],
...
e você assumiria que pagamento veio antes. *Mas isso é coincidência do relógio, não causalidade.*
...
#block(inset: (left: 1em), fill: luma(245), radius: 4pt, width: 100%)[
  Algoritmo descrito em Mattern (1989), Seção 3, e formalizado em Schwarz & Mattern (1994), "Detecting Causal Relationships in Distributed Computations".
]
...
#table(
  ...
  [*Problema*],
  [Last-write-wins (LWW)],
  [Quem "ganha" depende do clock da máquina],
  [Lock distribuído (Redis)],
  [Adiciona ponto de falha + latência],
  [Fila única (Kafka)],
  [Serializa tudo, perde throughput],
  [Ignorar (...)],
  [Bug intermitente que ninguém consegue reproduzir],
)
...
*Respostas:*
+ HAPPENS_BEFORE (a ≤ b em todas as dimensões, com P2 e P3 estritamente menor)
+ HAPPENS_AFTER (inverso do anterior)
+ CONCURRENT (P1: 3 #sym.lt 4 mas P2: 2 #sym.gt 1)
+ CONCURRENT (P2: 4 #sym.gt 1 mas P1: 3 #sym.lt 4 e P3: 2 #sym.lt 3)
...
#table(
  ...
  [Mesmo estado lógico],
  [INVERSO],
  [Inverte a perspectiva (A↔B)],
  [COMPARAR],
  [Algoritmo que compara dois vector timestamps],
)
