...
+ Determine `COMPARAR(vc_us, vc_eu)`. São causais ou concorrentes?

+ Se usássemos *last-write-wins com wall clock*, qual nome "ganharia"?
   Isso seria correto?

+ Se usássemos *Lamport Clock* em vez de Vector Clock, conseguiríamos
   detectar o conflito? Por quê?

+ Proponha *duas estratégias* para resolver esse conflito em produção.
...
- ts2 = {P:3, G:0, E:0} vs ts3 = {P:2, G:3, E:0}: *CONCURRENT*.
  ts2[P]=3 #sym.gt ts3[P]=2, mas ts2[G]=0 #sym.lt ts3[G]=3. São mensagens independentes
...
+ Modifique o pseudocódigo do middleware para usar Vector Clock em vez de
   Lamport Clock. Quais mudanças são necessárias?

+ O header agora é `X-Causality-Vector: order-svc=3,payment-svc=1`.
   Quanto maior fica o header conforme adicionamos serviços?
   Com 10 serviços, qual o tamanho aproximado?

+ Em quais pontos do seu sistema você adicionaria uma *checagem de concorrência*?
   (Dica: onde dois serviços podem escrever no mesmo recurso.)

+ Escreva o pseudocódigo de uma função `VERIFICAR_CONFLITO` que:
...
+ Cada entrada tem ~15-20 chars (ex: "payment-svc=12345").
...
#table(
  ...
  [Serviço reconstrói estado a partir de backup],
  [*MERGE*],
  [Reconstrução, não evento novo],
  [Health check pega snapshot de outro serviço],
  [*MERGE* (ou nada)],
  [Observação, não afeta causalidade],
)
